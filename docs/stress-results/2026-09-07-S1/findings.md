...
> **执行摘要（先读这里，下方为分轮实录）**
>
> 环境：3×4C15G 腾讯云（实际 15.6G/台），2+1' 拓扑，业务 systemd 直跑，SUT 本机自打
...
> 环境：3×4C15G 腾讯云 CVM（[../stress-machines.md](../stress-machines.md)），拓扑与编排见
...
- **实测：3×4C15G、2+1' 拓扑、SUT 本机自打，425,000 长连接同时在线**（135k/195k/95k），
...
- 简历/对外表述建议："3×4C15G 实测 42.5 万并发长连接（SUT 自打，含客户端开销；
